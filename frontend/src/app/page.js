    ...
    <div className="min-h-screen bg-[#0f0f0f] text-white font-sans">
      <header className="flex justify-between items-center px-6 py-4 border-b border-gray-800">
        ...
        <nav className="space-x-6 flex flex-row justify-around items-center text-gray-300 text-sm">
          {!loading && isAuthenticated ? (
            <Link href="/dashboard">
              <CgProfile size={36} color="white" />
            </Link>
          ) : null}
          {!loading && !isAuthenticated ? (
            <Link
              href="/login"
              className="bg-gradient-to-r from-blue-600 to-purple-600 px-5 py-2 text-white font-semibold rounded-full hover:opacity-90"
            >
              Login
            </Link>
          ) : (
            <button
              onClick={() => logout()}
              className="bg-gradient-to-r from-red-600 to-orange-600 px-5 py-2 text-white font-semibold rounded-full hover:opacity-90"
            >
              Logout
            </button>
          )}
      ...
      <section className="text-center py-24 px-6">
        ...
        <p className="text-gray-400 max-w-2xl mx-auto text-lg mb-10">
          Match up with coders worldwide in real-time. Solve the same challenge.
          First to finish wins.
        </p>
        <button
          onClick={() => handleRedirect()}
          className="inline-block bg-gradient-to-r from-green-500 to-teal-500 px-8 py-3 rounded-xl font-semibold text-black hover:opacity-90 transition-all duration-200"
        >
          Start a Match
        </button>
      </section>
      ...
      <section
        id="features"
        className="py-16 px-6 bg-[#111111] border-t border-gray-800"
      >
